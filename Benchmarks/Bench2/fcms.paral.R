...
for(i in 1:nrow(fcms.paral)) {
  ...
  fcms.paral[i, 1] <- fParal("dlbcl", arrays = fcms.paral[i, 2],
                             genes = fcms.paral[i, 3])
  
  assign(paste("fcms.paral.", numero, "cpu", sep = ""),
         fcms.paral)
  save(file = paste("fcms.paral.", numero, "cpu.RData", sep = ""),
       list = paste("fcms.paral.", numero, "cpu", sep = ""))
}
